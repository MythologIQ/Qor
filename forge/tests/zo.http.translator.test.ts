...
describe("Zo HTTP translator", () => {
  it("fails closed for ambiguous prompts", () => {
    const decision = toDecisionRequest({ input: "Do the thing." }, "did:myth:tester");
    expect(decision.action).toBe("execute");
  });

  it("uses model/session/context in request identity", () => {
    const a = toDecisionRequest(
      { input: "Summarize this.", model: "alpha", sessionId: "s1", context: { turn: 1 } },
      "did:myth:tester",
    );
    const b = toDecisionRequest(
      { input: "Summarize this.", model: "beta", sessionId: "s2", context: { turn: 2 } },
      "did:myth:tester",
    );
