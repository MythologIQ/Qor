...
function withRuntimeStub(
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (method === "POST" && url === "/evaluate") {
      ...
      res.setHeader("content-type", "application/json");
      res.end(
        JSON.stringify({ decision: "ALLOW", echo: JSON.parse(raw || "{}") }),
      );
      return;
    }
    res.statusCode = 404;
...
describe("QoreUiShellServer", () => {
  it("serves hub and evaluate proxy successfully", async () => {
    await withRuntimeStub(async (runtimeBaseUrl) => {
      ...
      const hub = (await hubRes.json()) as {
        qoreRuntime?: { connected?: boolean; policyVersion?: string };
      };
      ...
      // Verify Modern Monitor UI elements
      expect(consoleHtml.includes('id="phase-track"')).toBe(true);
      expect(consoleHtml.includes('id="sentinel-orb"')).toBe(true);
      expect(consoleHtml.includes('id="health-blockers"')).toBe(true);
      expect(consoleHtml.includes('class="qore-runtime-ribbon"')).toBe(true);
      expect(
        consoleHtml.includes(
          'data-tooltip="Real-time governance monitoring and system health"',
        ),
      ).toBe(true);

      // Verify legacy elements are NOT present
      expect(consoleHtml.includes('data-route="skills"')).toBe(false);
      expect(consoleHtml.includes('id="intent-template"')).toBe(false);
      ...
      const sec = (await secRes.json()) as {
        sessions?: { activeMfaSessions?: number };
      };
  ...
  it("enforces admin token and exposes session and MFA recovery controls", async () => {
    await withRuntimeStub(async (runtimeBaseUrl) => {
      ...
      const headers = {
        "x-qore-admin-token": "test-admin-token",
        "content-type": "application/json",
      };
      const security = await fetch(`${baseUrl}/api/admin/security`, {
        headers,
      });
      expect(security.status).toBe(200);
      const secBody = (await security.json()) as {
        auth?: { requireAdminToken?: boolean; adminTokenConfigured?: boolean };
      };
      ...
      const resetBody = (await mfaReset.json()) as {
        secret?: string;
        otpAuthUrl?: string;
      };
      expect((resetBody.secret ?? "").length).toBeGreaterThan(10);
      expect(String(resetBody.otpAuthUrl ?? "").startsWith("otpauth://")).toBe(
        true,
      );
  ...
  it("supports IDE panel embedding headers when enabled", async () => {
    await withRuntimeStub(async (runtimeBaseUrl) => {
      ...
      process.env.QORE_UI_FRAME_ANCESTORS =
        "'self' https://*.vscode-cdn.net vscode-webview:";
      ...
      const csp = String(
        monitorRes.headers.get("content-security-policy") ?? "",
      );
