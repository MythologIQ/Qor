...
export class InsightsPanel {
  ...
  renderHome(state, phase) {
    ...
    if (this.el.homeResource) {
      const verdictCount = (hub.recentVerdicts || []).length;
      this.el.homeResource.innerHTML = `
        <h3>Operational Pressure</h3>
        <div class="home-resource-row"><span>Recent Verdicts</span><strong>${verdictCount}</strong></div>
        <div class="home-resource-row"><span>Queue Depth</span><strong>${queueDepth}</strong></div>
        <div class="home-resource-row"><span>Sentinel</span><strong>${sentinelRunning ? 'Active' : 'Paused'}</strong></div>
      `;
    }
  ...
  renderGovernance(state) {
    ...
    this.el.sentinelStatus.innerHTML = `
      ...
      <div class="gov-health-bars">
        <div class="gov-bar-row">
          <div class="gov-bar-row-head"><span>Inference Integrity</span><strong>No data</strong></div>
          <div class="gov-bar integrity"><span style="width:0%"></span></div>
        </div>
        <div class="gov-bar-row">
          <div class="gov-bar-row-head"><span>Threat Detection</span><strong>${alerts.length > 0 ? alerts.length + ' alerts' : 'No alerts'}</strong></div>
          <div class="gov-bar threat"><span style="width:0%"></span></div>
        </div>
      </div>
      <div class="gov-stat-grid">
        <div class="gov-stat"><div class="label">Alerts</div><div class="value">${alerts.length}</div></div>
        <div class="gov-stat"><div class="label">Queue Depth</div><div class="value">${status.queueDepth || 0}</div></div>
      </div>
    `;
    ...
    this.el.trustSummary.innerHTML = `
      ...
      <div class="gov-policy-row"><span>AGENTS</span><span class="policy-state">${trust.totalAgents || 'No data'}</span></div>
      <div class="gov-policy-row"><span>AVG TRUST</span><span class="policy-state">${trust.avgTrust > 0 ? trust.avgTrust.toFixed(2) : 'No data'}</span></div>
      <div class="gov-policy-row"><span>QUARANTINED</span><span class="policy-state">${trust.quarantined || 0}</span></div>
    `;
