...
test("queueAgent adds to correct bracket", async () => {
  ...
  const { agent: ag1 } = registerAgent(op1.id, "TestAgent-A", "fp-a", "qwen2.5-14b");
  const { agent: ag2 } = registerAgent(op2.id, "TestAgent-B", "fp-b", "gpt-4o");
...
test("findOpponent excludes self", async () => {
  ...
  const { agent: ag3 } = registerAgent(op3.id, "TestAgent-C", "fp-c", "llama-70b");
...
test("launchMatch creates MatchRecord and clears queue", async () => {
  ...
  const { agent: ag4 } = registerAgent(op4.id, "LaunchAgent-A", "fp-la", "qwen2.5-7b");
  const { agent: ag5 } = registerAgent(op5.id, "LaunchAgent-B", "fp-lb", "gpt-4o-mini");
  ...
  expect(match.agentAId).toBe(ag4.id);
  expect(match.agentBId).toBe(ag5.id);
...
test("bracket auto-classification via model size", async () => {
  ...
  const { agent: ag_s7 } = registerAgent(op.id, "SentinelAgent", "fp-s7", "qwen2.5-7b");
  const { agent: ag_v14 } = registerAgent(op.id, "VanguardAgent", "fp-v14", "qwen2.5-14b");
  const { agent: ag_apex } = registerAgent(op.id, "ApexAgent", "fp-apex", "claude-3-5-sonnet");
...
test("creates a match when two agents are enqueued in the same bracket", async () => {
  ...
  const { agent: ag1 } = registerAgent(op1.id, "TestAgent-2A", "fp-2a", "gpt-4o-mini");
  const { agent: ag2 } = registerAgent(op2.id, "TestAgent-2B", "fp-2b", "qwen2.5-14b");
