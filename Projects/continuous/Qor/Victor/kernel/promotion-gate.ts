...
export async function summarizeSoakEvidence(options?: {
  ...
  const checks: SoakEvidenceCheck[] = [
    ...
    evaluateCheck(
      ...
      tickRecords.length > 0 && tickRecords.every((record) => stringOrNull(record.payload.selectedTaskId)),
      distinctSelectedTaskIds.length > 0,
      'At least one soak tick completed without an explicit selected task id.',
  ...
  return {
    ...
    heartbeat: {
      ...
      tickCount: state?.tickCount ?? tickRecords.length,
...
async function listHeartbeatLedgerRecords(
  ...
  const ledger = createPlanningLedger(projectId, projectsDir);
  ...
  return entries
    .filter((entry) => entry.payload?.source === 'victor-heartbeat')
...
async function readPathPhases(projectId: string, projectsDir: string): Promise<Array<Record<string, unknown>>> {
  ...
  const payload = await pathStore.read<{ phases?: Array<Record<string, unknown>> }>();
