...
import type { HelloFrame, StateFrame, AckFrame, EndFrame } from '../../src/gateway/contract';

describe('WebSocket Handler', () => {
  describe('buildHelloFrame', () => {
    test('produces valid HelloFrame', () => {
      const frame = buildHelloFrame('match-1', 'A', 'seed-abc', { width: 7, height: 7 }, 5000, 150);
      ...
      expect(frame.turnCap).toBe(150);
      expect(frame.protocolVersion).toBe('1.0');
    });
  });

  describe('buildStateFrame', () => {
    test('produces valid StateFrame', () => {
      const frame = buildStateFrame(5, true, [], [], { a: 10, b: 8 }, Date.now() + 5000);
      expect(frame.type).toBe('STATE');
      expect(frame.turn).toBe(5);
      expect(frame.yourTurn).toBe(true);
  ...
  describe('buildAckFrame', () => {
    ...
    test('accepted=false with reason', () => {
      const frame = buildAckFrame(false, 'invalid_action');
      expect(frame.accepted).toBe(false);
      expect(frame.reason).toBe('invalid_action');
    });
  });
  ...
  describe('parseFrame', () => {
    ...
    test('parses ACTION frame', () => {
      const raw = JSON.stringify({ type: 'ACTION', action: 'move', from: { q: 0, r: 0, s: 0 }, to: { q: 1, r: -1, s: 0 }, confidence: 0.95 });
      const frame = parseFrame(raw);
