...
def solve_challenge(challenge):
    ...
    if any(w in clean for w in ['total', 'adds', 'plus', 'accelerat', 'sum', 'how much']):
        return a + b
    elif any(w in clean for w in ['loses', 'slow', 'decelerat', 'minus', 'subtract', 'new speed', 'new velocity']):
        return a - b
...
def post_and_verify(post_id, content, label):
    ...
    if ans is not None:
        answer_str = f"{ans:.2f}"
        print(f"  Challenge solved: {answer_str}")
...
print(f"\n=== RESULTS: {verified}/{posted} verified ===")
