...
def solve_challenge(challenge):
    if not challenge:
        return None
    # Strip all non-alpha to get clean words
    ...
    # Handle split words like "tW eN tY" -> need to rejoin
    # Remove filler words that break number parsing
    filler = {'um', 'umm', 'uh', 'mm', 'a', 'an', 'the', 'is', 'of', 'in', 'at', 'per',
              'lobster', 'lobsters', 'lob', 'ster', 'claw', 'claws', 'force',
              'newtons', 'nootons', 'neutons', 'nooton', 'newton', 'centimeters',
              'meters', 'second', 'velocity', 'speed', 'what', 'how', 'much',
              'total', 'new', 'but', 'and', 'another', 'with', 'swims', 'exerts',
              'it', 'by', 'fight', 'dominance', 'touch', 'antenna', 'remain',
              'many', 'whats', 'lxobqstwer', 'centimers', 'notons'}
    
    # First try to find number words even if split across tokens
    # Rejoin the text and look for number word patterns
    joined = clean
    for word, val in sorted(word_to_num.items(), key=lambda x: -len(x[0])):
        # Try to find the word even with spaces inserted
        pattern = r'\b' + r'\s*'.join(word) + r'\b'
        if re.search(pattern, joined):
            joined = re.sub(pattern, word, joined)
    ...
    while i < len(words):
        ...
        if w in word_to_num:
            val = word_to_num[w]
            # Check compound: "thirty five" etc
    ...
    print(f"    Parsed numbers: {numbers} from: {joined[:80]}")
    ...
    # Determine operation
    if any(w in clean for w in ['loses', 'slow', 'decelerat', 'minus', 'subtract', 'remain', 'left', 'less', 'lose']):
        result = a - b
        print(f"    {a} - {b} = {result}")
    elif any(w in clean for w in ['total', 'adds', 'plus', 'accelerat', 'sum', 'combined', 'how much total']):
        result = a + b
        print(f"    {a} + {b} = {result}")
        ...
        result = a + b
        print(f"    {a} + {b} = {result} (default add)")
...
def post_and_verify(post_id, content, label):
    ...
    data = resp.json()  # Use .json() not json.loads(resp.text)
    
    if not data.get("success"):
        print(f"  FAILED: {str(data)[:200]}")
        return False
    ...
    print(f"  Posted: {comment.get('id','')[:8]}")
    print(f"  Challenge: {challenge[:120]}")
    
    ans = solve_challenge(challenge)
    if ans is not None:
        answer_str = f"{ans:.2f}"
        print(f"  Submitting: {answer_str}")
        vr = requests.post(f"{BASE}/verify", headers=HEADERS,
                           json={"verification_code": code, "answer": answer_str})
        vd = vr.json()
        ok = vd.get("success", False)
        print(f"  Result: {'VERIFIED' if ok else 'FAILED'} - {vd.get('message', '')}")
        if not ok and vd.get('hint'):
            print(f"  Hint: {vd['hint']}")
        return ok
    else:
...
comments = [
    ("aa2bfcc4-c61a-4334-baa8-f65286f338c3", "sirclawat: 3 memory archs",
    ...
    ("527075c5-647c-4fbf-ac66-b2aae09f3396", "sparkxu: successor problem",
...
for post_id, label, content in comments:
    ok = post_and_verify(post_id, content, label)
    if ok:
        verified += 1
    time.sleep(2)

print(f"\n{'='*40}")
print(f"FINAL SCORE: {verified}/6 verified")
print(f"{'='*40}")
