...
app.get('/api/victor/memory/readiness', async (c) => {
  try {
    const kernel = await getMemoryKernel().catch((error) => {
      throw {
        readinessError: true,
    ...
    const summary = await assessMemoryAutomationReadiness({
      ...
      documentsRoot: c.req.query('documentsRoot') ?? undefined,
      builderProjectId: c.req.query('builderProjectId') ?? undefined,
      researchProjectId: c.req.query('researchProjectId') ?? undefined,
      ingestBuilderArtifacts: c.req.query('ingestBuilderArtifacts') === 'false' ? false : undefined,
      ingestDocuments: c.req.query('ingestDocuments') === 'false' ? false : undefined,
    });
    const status =
      summary.overall === 'ready'
        ? 200
        : summary.overall === 'conditionally-ready'
          ? 202
          : 422;
    return c.json({
      status: 'ok',
    ...
    if (typeof error === 'object' && error !== null && 'readinessError' in error) {
      return c.json(
        {
          status: 'ok',
          summary: {
            ...
            builderProjectId: c.req.query('builderProjectId') ?? 'builder-console',
            researchProjectId: c.req.query('researchProjectId') ?? 'victor-research',
            criteria: [
              {
                id: 'evaluation-suite',
    ...
    return c.json(
      {
        error: error instanceof Error ? error.message : 'Memory readiness check failed',
      },
      500,
    );
  }
    ...
    const result = await runGovernedAutomationBuild(
      {
        ...
        actorId: request.actorId,
        maxActions: request.maxActions,
        multiProject: request.multiProject ?? false,
      },
      groundedQuery,
    );
...
async function resolveGroundedQuery(request: HeartbeatRequest | { projectId?: string; projectsDir?: string; dryRun?: boolean; actorId?: string }) {
  try {
    ...
    const fallbackRequest = request as { projectId?: string; projectsDir?: string; allowWorkspaceFallback?: boolean };
    if (fallbackRequest.allowWorkspaceFallback && fallbackRequest.projectsDir) {
      const { createWorkspaceGroundedQuery } = await import('./workspace-grounded-query');
      return createWorkspaceGroundedQuery(fallbackRequest.projectsDir, fallbackRequest.projectId || 'victor-resident');
    }
    throw new Error(
      `Live Victor memory is required for heartbeat and governed-build automation endpoints. ${
