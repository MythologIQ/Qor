...
import type { ExplorationConfig, ExplorationState, ExplorationResult } from './memory/exploration.js';
import { runIdleExploration, createExplorationState, DEFAULT_EXPLORATION_CONFIG } from './memory/exploration.js';
...
export interface HeartbeatRequest {
  ...
  /** Exploration configuration for idle-time content fetching */
...
export interface HeartbeatState {
  ...
  /** Exploration state for idle-time content fetching */
...
export interface HeartbeatTickResult {
  ...
  /** Result of idle exploration if performed */
  ...
  try {
    ...
    checks.push({
      ...
      detail: selectedTask
        ...
        : `Path state is readable with ${phases.length} phases; no governed automation task is currently available, so heartbeat may record a cadence-only tick.`,
    });
  } catch (error) {
    checks.push({
      name: 'path-state',
  ...
  try {
    if (!selectedTask) {
      checks.push({
        name: 'governance',
        passed: true,
        detail: 'No target task is available; governance preflight accepts a cadence-only heartbeat tick.',
      });
  ...
  try {
    if (!selectedTask) {
      checks.push({
        name: 'grounded-query',
        passed: true,
        detail: 'No target task is available; grounded-query preflight is skipped for cadence-only heartbeat ticks.',
      });
    } else {
...
export async function tickHeartbeat(
  ...
  const runningState: HeartbeatState = {
    ...
    mode: contract.mode,
    reasoningModel: contract.reasoningModel,
    baselineCadenceMs: contract.baselineCadenceMs,
    staleAfterMs: contract.staleAfterMs,
    maxActionsPerTick: contract.maxActionsPerTick,
    stopOnBlock: contract.stopOnBlock,
    maxConsecutiveBlocked: contract.maxConsecutiveBlocked,
  ...
  try {
    ...
    // Run idle exploration if automation is blocked
    let explorationResult: ExplorationResult | undefined;
    if (automation.status === 'blocked' && contract.exploration.enabled) {
      try {
        const exploreResult = await runIdleExploration(contract.exploration);
        explorationResult = {
          didExplore: exploreResult.explored,
          items: [], // Simplified for integration
          actionCounts: { admit: 0, 'sanitize-admit': 0, quarantine: 0, reject: 0 },
          hasUsefulContent: exploreResult.hasUsefulContent,
          state: exploreResult.state,
          errors: exploreResult.errors,
        } as ExplorationResult;

        // Update running state exploration
        runningState.exploration = exploreResult.state;

        // If exploration produced useful content, don't increment blocked counter
        if (exploreResult.hasUsefulContent) {
          runningState.exploration.consecutiveExploreTicks = 0;
        }
      } catch (exploreError) {
        // Exploration errors don't stop heartbeat, just log
        const errorMsg = exploreError instanceof Error ? exploreError.message : String(exploreError);
        // Continue with normal blocked handling
      }
    }
    ...
    const isCadenceOnlyTick =
      !automation.selectedTask
      && automation.selectionReason === 'No eligible governed automation task was found.';
    const tickStatus: HeartbeatTickResult['status'] = isCadenceOnlyTick ? 'completed' : automation.status;
    const tickReason = isCadenceOnlyTick
      ? 'Cadence-only tick recorded; no eligible governed automation task was found.'
      : (automation.selectionReason || automation.automation.results[0]?.reason || null);

    // Calculate consecutiveBlocked - reset if not blocked or if exploration produced content
    let nextConsecutiveBlocked = runningState.consecutiveBlocked;
    if (tickStatus === 'blocked') {
      // Don't increment if exploration produced useful content
      if (!explorationResult?.hasUsefulContent) {
        nextConsecutiveBlocked = runningState.consecutiveBlocked + 1;
      }
    } else {
      nextConsecutiveBlocked = 0;
    }
    ...
    const nextState = {
      ...
      consecutiveBlocked: nextConsecutiveBlocked,
      consecutiveFailures: 0,
    } satisfies HeartbeatState;
    const reflectedState = applyCooldownCycle(nextState, tickStatus);
    ...
    return {
      ...
      reflectiveNote,
      exploration: explorationResult,
      reason: finalState.lastTickReason || tickStatus,
    };
  } catch (error) {
    const failedState = {
      ...runningState,
...
function resolveHeartbeatContract(request: HeartbeatRequest): HeartbeatContract {
  ...
  return {
    projectId: request.projectId?.trim() || DEFAULT_AUTOMATION_PROJECT_ID,
    ...
    exploration: {
      ...
      enabled: (request.exploration?.enabled === true) && !!process.env.MOLTBOOK_API_KEY,
    },
  };
}
...
function buildHeartbeatQuery(task: BuilderConsoleTask, phases: BuilderConsolePhase[]) {
  ...
  return [
    `For the governed automation task "${task.title}" in phase "${phaseName}", choose the single highest-value next safe action Victor should take now.`,
    `Prefer advancing the task directly. If direct advancement is not justified, choose the most concrete unblock action that creates progress inside Builder Console.`,
    `Return guidance that can support exactly one governed action or one explicit no-action veto with a named blocker.`,
    `Do not recommend passive observation, generic monitoring, or "continue current work" unless you also identify the specific blocker, missing artifact, or dependency that prevents safe execution.`,
  ].join(' ');
}
