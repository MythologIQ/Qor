...
describe('heartbeat foundation', () => {
  ...
  it('persists execute mode when a live tick runs against an existing heartbeat lease', async () => {
    const started = await startHeartbeat(request(projectsDir, stateDir), groundedQueryResolver);
    expect(started.status).toBe('started');
    expect(started.state?.mode).toBe('dry-run');
    ...
    expect(tick.status).toBe('completed');
    expect(tick.automation?.mode).toBe('execute');
    expect(tick.automation?.automation.executedCount).toBe(1);
    expect(tick.state.mode).toBe('execute');
  ...
  it('allows cadence-only heartbeat operation when no governed tasks remain', async () => {
    const statePath = join(projectsDir, 'builder-console', 'path', 'phases.json');
    const raw = JSON.parse(await readFile(statePath, 'utf8'));
    raw.phases[0].tasks = raw.phases[0].tasks.map((task: Record<string, unknown>) => ({
      ...task,
    ...
    const preflight = await runHeartbeatPreflight(request(projectsDir, stateDir), groundedQueryResolver);
    expect(preflight.ok).toBe(true);
    expect(preflight.checks.find((check) => check.name === 'path-state')?.detail).toContain('cadence-only');
    ...
    expect(tick.status).toBe('completed');
    expect(tick.state.tickCount).toBe(1);
    expect(tick.state.lastTickStatus).toBe('completed');
    expect(tick.state.consecutiveBlocked).toBe(0);
    expect(tick.state.lastTickReason).toContain('Cadence-only tick recorded');

    const status = await getHeartbeatStatus(request(projectsDir, stateDir));
    expect(status?.status).toBe('active');
    expect(status?.lastTickCompletedAt).toBeString();
  });

  it('drafts a follow-on governed task when the authored queue is exhausted but project evidence supports a next step', async () => {
    const statePath = join(projectsDir, 'builder-console', 'path', 'phases.json');
    const raw = JSON.parse(await readFile(statePath, 'utf8'));
    raw.phases[0].tasks = raw.phases[0].tasks.map((task: Record<string, unknown>) => ({
      ...task,
    ...
    const tick = await tickHeartbeat(
      request(projectsDir, stateDir),
      async (_contract: HeartbeatContract) => async (_projectId: string, query: string): Promise<GroundedContextBundle> => {
        if (query.includes('currently authored phases are complete')) {
          return groundedContext(
            query,
            ['Draft governed Moltbook intake prerequisites for Victor heartbeat'],
          );
        }
        return groundedContext(query);
      },
    );

    expect(tick.status).toBe('completed');
    expect(tick.state.lastTickReason).toContain('Grounded project-state derivation proposed follow-on task');
    expect(tick.automation?.automation.results[0]?.kind).toBe('create-draft-task');
    expect(tick.automation?.automation.results[0]?.status).toBe('eligible');
    expect(tick.state.lastSelectedTaskId).toContain('derived-');
  });
