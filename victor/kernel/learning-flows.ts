...
export class DebugLearningFlow {
  constructor(private knowledgeGraph: LearningStore) {}
...
export class SubstantiateLearningFlow {
  ...
  async consolidate(
    ...
    const packet = LearningPacketValidator.toSubstantiatePacket({
      ...
      actual: reality,
      lessons,
      timeline_delta: delta.timeline_delta,
  ...
  private analyzeDeltas(plan: any, reality: any): string[] {
    const deltas: string[] = [];

    if (reality.timeline > plan.timeline) {
      deltas.push('Timeline exceeded estimate');
    }

    if (reality.quality < plan.quality) {
      deltas.push('Delivered quality below target');
    }

    if (Array.isArray(reality.unexpectedIssues) && reality.unexpectedIssues.length > 0) {
      deltas.push('Unexpected issues introduced unplanned work');
    }
