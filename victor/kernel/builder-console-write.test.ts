...
describe('createGovernedBuilderConsoleDraftTask', () => {
  ...
  it('blocks task creation when recall governance is advisory', async () => {
    const result = await createGovernedBuilderConsoleDraftTask(
      {
        projectsDir,
        projectId: 'builder-console',
        phaseId: 'phase-1',
        title: 'Advisory recall write',
        description: 'This should fail because recall is not grounded.',
        query: 'What is the authority model for Victor?',
      },
      {
        ...groundedContext(),
    ...
    expect(result.status).toBe('blocked');
    expect(result.reason).toContain('recall governance is advisory');
  });

  it('blocks task creation when unresolved negative constraints are present', async () => {
    const result = await createGovernedBuilderConsoleDraftTask(
      {
        projectsDir,
        projectId: 'builder-console',
        phaseId: 'phase-1',
        title: 'Constraint violating write',
        description: 'This should fail because failure memory still constrains the action.',
        query: 'authority model',
      },
      {
        ...groundedContext(),
        missingInformation: ['Unresolved negative constraint: AVOID: collapsing contradictory authority claims into one answer'],
      },
    );

    expect(result.status).toBe('blocked');
    expect(result.reason).toContain('unresolved failure memory constrains the action');
  });
...
describe('updateGovernedBuilderConsoleTaskStatus', () => {
  ...
  it('blocks task status updates when recall governance is advisory', async () => {
    const result = await updateGovernedBuilderConsoleTaskStatus(
      {
        projectsDir,
        projectId: 'builder-console',
...
describe('appendGovernedBuilderConsolePlanningNote', () => {
  ...
  it('blocks reflective planning note writes when recall governance is blocked', async () => {
    const result = await appendGovernedBuilderConsolePlanningNote(
      {
        projectsDir,
        projectId: 'builder-console',
    ...
    expect(result.status).toBe('blocked');
    expect(result.reason).toContain('recall governance is blocked');
  });
});
