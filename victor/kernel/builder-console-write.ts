...
import { extractUnresolvedNegativeConstraints } from './memory/failure-memory';
...
function validateRequest(
  ...
  const recallBlock = validateRecallDecision(groundedContext);
  if (recallBlock) {
    return blocked(`Victor refused draft task creation because recall governance is ${groundedContext.recallDecision?.mode}: ${recallBlock}`);
  }

  const negativeConstraintBlock = unresolvedNegativeConstraintMessage(groundedContext);
  if (negativeConstraintBlock) {
    return blocked(`Victor refused draft task creation because unresolved failure memory constrains the action: ${negativeConstraintBlock}`);
  }
...
function validateTaskStatusRequest(
  ...
  const recallBlock = validateRecallDecision(groundedContext);
  if (recallBlock) {
    return blockedStatusUpdate(`Victor refused task status update because recall governance is ${groundedContext.recallDecision?.mode}: ${recallBlock}`);
  }

  const negativeConstraintBlock = unresolvedNegativeConstraintMessage(groundedContext);
  if (negativeConstraintBlock) {
    return blockedStatusUpdate(`Victor refused task status update because unresolved failure memory constrains the action: ${negativeConstraintBlock}`);
  }
...
function validateReflectivePlanningNoteRequest(
  ...
  const recallBlock = validateRecallDecision(groundedContext);
  if (groundedContext.recallDecision?.mode === 'blocked') {
    return blockedPlanningNote(`Victor refused reflective planning note creation because recall governance is blocked: ${recallBlock}`);
  }
...
function validateRecallDecision(groundedContext: GroundedContextBundle): string | null {
  if (!groundedContext.recallDecision) {
    return null;
  }

  if (!groundedContext.recallDecision.allowed || groundedContext.recallDecision.mode === 'blocked') {
    return groundedContext.recallDecision.reason;
  }

  if (groundedContext.recallDecision.mode === 'advisory') {
    return groundedContext.recallDecision.reason;
  }
...
function unresolvedNegativeConstraintMessage(groundedContext: GroundedContextBundle): string | null {
  return extractUnresolvedNegativeConstraints(groundedContext)[0] ?? null;
}
