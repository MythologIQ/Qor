...
import { createSourceDocument, hashContent, createUORChunkId } from './provenance';
...
export function planArtifactIngestion(input: DocumentInput, snapshot: DocumentSnapshot): IngestionPlan {
  ...
  const chunks = chunkDocument(document, input.content).map((chunk) => ({
    ...chunk,
    uorId: createUORChunkId(document.uorId!, chunk.span, hashContent(chunk.text)),
    parentDocUorId: document.uorId,
    governance: createGovernanceMetadata('sourceChunk', {
      rationale: 'Source chunk is directly anchored to a fingerprinted workspace document with UOR identity.',
    }),
  ...
  return {
    ...
    ingestionRun: {
      ...
      governance: createGovernanceMetadata(undefined, {
        ...
        confidence: 0.85,
        rationale: 'Ingestion run records a governed memory mutation event.',
      }),
    },
  };
}
