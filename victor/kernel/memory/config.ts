...
const LOCAL_NEO4J_HOME = '/home/workspace/Projects/continuous/Victor/kernel/.runtime/neo4j-community-5.26.6';
const LOCAL_DEV_PASSWORD = 'victor-memory-dev';
...
export function loadNeo4jConfig(env: Record<string, string | undefined> = process.env): Neo4jConfig {
  ...
  const password = env.NEO4J_PASSWORD?.trim()
    || (
      env === process.env
      && env.VICTOR_ALLOW_LOCAL_NEO4J_FALLBACK !== 'false'
      && existsSync(LOCAL_NEO4J_HOME)
        ? LOCAL_DEV_PASSWORD
        : undefined
    );
